...
# 24 "Print_functions.c" 2
# 1 "Yahtzee_global_vars.h" 1
# 73 "Yahtzee_global_vars.h"

# 73 "Yahtzee_global_vars.h"
extern int score[][13];
# 92 "Yahtzee_global_vars.h"
extern char names[][18];
...
# 25 "Print_functions.c" 2
# 1 "Print_functions.h" 1
...
void resetScreen();
void printDice();
# 26 "Print_functions.c" 2
# 39 "Print_functions.c"
void prntHorisontalSeperator(int n);
# 49 "Print_functions.c"
void printScoreSheet(){
 ...
 printHorisontalSeparator( 20 + n_players * (18 +2) );
 printf("\n");
...
void printDice(){
 ...
 printf("|");
 for( i = 0; i < 5; ++i ){
  printf("\x1b[35m""%8d""\x1b[0m""  |", dice[i]);
 }
...
# 82 "Yahtzee.c"
  ...
  if( n_players > 0 && n_players <= 4 ) break;
...
void playRound(){
 ...
 for( player = 0; player < n_players; ++player){
  ...
  while(1){
   ...
   if( roll < 3 -1 ) {
    printf("You have %d re-rolls left!", 3 - roll - 1);
   } else {
...
# 185 "Yahtzee.c"
  ...
  is_err_flag = 0;
  for( i = 0; line[i] != '\n' && line[i] != '\0'; ++i ){
   if( line[i] < 'a' || line[i] > 'a' + 5 ){
    printf("\x1b[31m" "Unknown dice!\n" "\x1b[0m");
    is_err_flag = 1;
    break;
   }
  }
...
int getScore( int category ){
 ...
 switch( category ){
  ...
  case 12:
   ...
   for( i = 0; i < 5; ++i ) points += dice[i];
   break;
  default:
   printf("\x1b[31m" "Ups! Something Went Wrong!!!\n" "\x1b[0m");
   break;
 }
...
# 531 "Yahtzee.c"
  ...
  if( category >= 13 || category < 0){
   printf("\x1b[31m" "You may select categories from 1 to %d!\n" "\x1b[0m", 13);
   continue;
  }
  ...
  printf("\x1b[31m" "U can not reuse categories!\n" "\x1b[0m");
 }
